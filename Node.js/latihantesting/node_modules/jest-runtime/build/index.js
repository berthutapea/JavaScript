...
const runtimeSupportsImportAssertions = (0, _semver().satisfies)(
...
class Runtime {
  ...
  async loadEsmModule(modulePath, query = '', importAssertions) {
  ...
  validateImportAssertions(
    ...
    const url = (0, _url().pathToFileURL)(modulePath);
    if (query) {
      url.search = query;
    }
  ...
  async linkAndEvaluateModule(module) {
    ...
    if (module.status === 'unlinked') {
      ...
      this._esmModuleLinkingMap.set(
        module,
        module.link((specifier, referencingModule, importCallOptions) =>
          this.resolveModule(
            specifier,
            referencingModule.identifier,
            referencingModule.context,
  ...
  async _importWasmModule(source, identifier, context, importAssertions) {
...
function handleInvalidAssertionType(url, type) {
  ...
  if (!supportedAssertionTypes.has(type)) {
    const error = new Error(`Import assertion type "${type}" is unsupported`);
    error.code = 'ERR_IMPORT_ASSERTION_TYPE_UNSUPPORTED';
    throw error;
  }

  // `type` was the wrong value for this format.
  const error = new Error(`Module "${url}" is not of type "${type}"`);
